...
from sauron.tools import register_observation
...
async def observe_tile(
    ...
    ----------
    sauron
        A `.Sauron` instance. If not provided, a new one will be created.
    tile_id
    ...
    if not sauron:
        sauron = await Sauron().init()

    if verbose:
        sauron.set_verbosity("debug")

    tile_id_data = await sauron.telescopes.goto_tile_id(tile_id)
    ...
    exp_nos = await sauron.specs.expose(tile_data=exp_tile_data, show_progress=True)
